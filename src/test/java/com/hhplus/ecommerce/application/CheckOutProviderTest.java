...
//        Long catStock = 10L;
//        stocks.add(new StockCommand(catId, catStock));
...
//                try {
//                    checkOutProvider.checkOut(null);
//                    successCount.getAndIncrement();
//                } catch (InterruptedException | InputValidationException |
//                        ConcurrentModificationException | IllegalMonitorStateException e) {    failCount.getAndIncrement();}
//                finally {
...
//        assertThat(successCount.get()).isEqualTo(numThreads < catStock ? numThreads : catStock);
//
//
//        assertThat(result.getProductDetail().getQuantity()).isEqualTo( catStock - ((long) successCount.get()*1L));
//
//        assertThat(successCount.get()).isEqualTo(numThreads < catStock ? numThreads : catStock);
//        assertThat(failCount.get()).isEqualTo(numThreads - catStock < 0 ? 0 : numThreads - catStock);
//    }
//
//    @Test
...
//            checkOutProvider.checkOut(null);
//            successCount.getAndIncrement();} catch (InterruptedException | InputValidationException |
//                                                    ConcurrentModificationException e) {    failCount.getAndIncrement();} finally {    doneSignal.countDown();}
//        });
//        executorService.execute(() -> { try {
//            checkOutProvider.checkOut(null);
//            successCount.getAndIncrement();} catch (InterruptedException | InputValidationException |
